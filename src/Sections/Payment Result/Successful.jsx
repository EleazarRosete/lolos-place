        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                if (firstData.paidorder && firstData.order && firstData.salesdata) {
                    ...
                    let orders = Array.isArray(firstData.order) ? firstData.order : [firstData.order];
                    console.log(orders);
                    ...
                    if (!paidorderResponse.ok) {
                        const errorData = await paidorderResponse.text(); // Change to .text() to handle non-JSON responses
                        console.error("Error response from adding order:", errorData);
                        throw new Error(`Error adding order data: ${errorData}`);
                    }
                    ...
                    const salesPromises = paidOrderData.items.map(async (orderedItem) => {
                        ...
                        if (product) {
                            ...
                            if (!salesResponse.ok) {
                                const errorData = await salesResponse.text(); // Change to .text() to handle non-JSON responses
                                console.error("Error response from adding sales:", errorData);
                                throw new Error(`Error adding sales data: ${errorData}`);
                            }
